...
Select region_id , count(node_id) as node_per_region
from customer_nodes
group by region_id order by region_id;

/* How many customers are allocated to each region? */
Select region_id, count(distinct customer_id) as customer_per_region
from customer_nodes
group by region_id 
order by region_id;

/*How many days on avearge are customers rellocated to a different node?*/
Select Avg(datediff(end_date, start_date)) as avg_reallocation_days
from customer_nodes
where end_date != '9999-12-31';
/*What is the median, 80th and 95th percentile for this same rellocation days metric for each region?*/
With days_cte as (
	select c.region_id,
			r.region_name,
            datediff(end_date, start_date) as reallocation_days,
            Row_Number() over(partition by r.region_name order by datediff(end_date, start_date)) as row_num,
...
Select 
        ...
        Max(Case When row_num = Ceil(.5*total_rows) Then reallocation_days end) As median,
        max(case when row_num = Ceil(.8 * total_rows) Then reallocation_days end) As percentile_80,
        Max(case when row_num = Ceil(.95 * total_rows) Then reallocation_days end) As percentile_95
        From days_cte
...
/*What is the avg total historical deposits counts and amounts for all customers?*/
With deposits_summary as(
	select customer_id , txn_type,
    count(case when txn_type = 'deposit' Then 1 else 0 End) as total_count,
    sum(case when txn_type = 'deposit' Then txn_amount End) as total_amount
    From customer_transactions
    group by customer_id , txn_type )
Select txn_type,
Avg(total_count) as avg_deposit_count,
Avg(total_amount) as avg_deposit_amount
from deposits_summary
where txn_type = 'deposit'
group by txn_type;

/*For each month - How many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawl in a single month?*/
With count_cte as (
	Select customer_id, 
			Date_Format(txn_date, '%b') as Months,
            ...
            Count(case when txn_type = 'withdrawl' then 1 else 0 End) as withdrawl_count
		From customer_transactions
...
Select Count(Distinct customer_id) as customer_count,
        ...
        And (purchase_count > 0 Or withdrawl_count > 0)
        group by Months
        order by customer_count DESC;


/* What is the closing balance for each customer for each month?*/
With cte_amount as (
 select customer_id,
...
Select customer_id, months,
		sum(amount) over (partition by customer_id order by months 
							Rows between unbounded preceding and current row) as closing_balance
		from cte_amount
        group by customer_id, months, amount
        order by customer_id;
...
select customer_id, months, closing_balance,
		100 * (closing_balance - lag(closing_balance) over (partition by customer_id order by months))/ 
        NULLIF(lag(closing_balance) over(partition by customer_id order by months), 2) as percentage_increase
	from closing_balance)
Select 100 * Count(Distinct customer_id) / (select count(Distinct customer_id) from customer_transactions) 
